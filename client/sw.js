...
const CACHE_NAME    = 'face-ai-tracker-v12';   // ← bumped from v3
...
self.addEventListener('install', function(event) {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then(function(cache) {
      console.log('[SW v12] Caching core files');
      ...
      return Promise.allSettled(
        CORE_FILES.map(function(url) {
          return fetch(url).then(function(response) {
            ...
            console.warn('[SW v12] Could not cache:', url, e.message);
          });
        })
      );
    }).then(function() {
      ...
      console.log('[SW v12] Install complete — waiting for activation signal');
    })
  );
});
